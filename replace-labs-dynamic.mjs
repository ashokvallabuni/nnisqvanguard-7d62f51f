...
if (!content.includes('LAB_DEFINITIONS')) {
  content = content.replace('import { LabCard, LabData } from "@/components/cyber-range/LabCard";', importStatement + 'import { LabCard, LabData } from "@/components/cyber-range/LabCard";');
}

const canonicalLabsRegex = /\/\/ Canonical defense & threat investigation labs\s+const CANONICAL_LABS: LabData\[\] = \[[\s\S]*?\];/;
content = content.replace(canonicalLabsRegex, '// Canonical defense & threat investigation labs\nconst CANONICAL_LABS: LabData[] = LAB_DEFINITIONS as LabData[];');
...
if (!labCardContent.includes('courseId?: string;')) {
  labCardContent = labCardContent.replace('summary: string;', 'summary: string;\n  courseId?: string;');
