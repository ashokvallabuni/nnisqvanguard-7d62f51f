...
test.describe("IVVAB LABS Core Verification", () => {
  test("Virtual Filesystem & Terminal Execution", async ({ page }) => {
    ...
    await expect(
      page.getByText("EXPLORE IVVAB LABS").or(page.getByText("IVVAB LABS").first()),
    ).toBeVisible();

    // Start the Linux Lab
    const startButton = page
      .getByRole("link", { name: /Linux SSH Brute Force/i })
      .or(page.locator('a[href="/labs/linux-ssh-bruteforce"]'));
