...
const SOC_STAGES: PipelineStage[] = [
  {
    ...
    exampleData:
      "Sep 21 10:14:02 auth-gw sshd[18442]: Failed password for root from 198.51.100.44 port 48212 ssh2",
    analystAction:
      "Verify log collection health, ensure time synchronization via NTP, and validate agent telemetry heartbeat.",
  },
  {
    step: 2,
    ...
    exampleData:
      '{"event.category": "authentication", "event.outcome": "failure", "source.ip": "198.51.100.44", "user.name": "root"}',
    analystAction:
      "Audit parser regex accuracy and ensure geo-IP enrichment pipelines are functioning properly.",
  },
  {
    step: 3,
    ...
    exampleData:
      "RULE_TRIGGER: Count(event.outcome == 'failure') > 100 within 60s WHERE user == 'root'",
    analystAction:
      "Tune detection thresholds to eliminate false positives from automated service accounts.",
  },
  {
    step: 4,
    ...
    analystAction:
      "Acknowledge alert in the ticketing queue within the defined SLA response window (e.g. < 15 min).",
  },
  {
    step: 5,
    ...
    exampleData:
      "Finding: Attacker IP 198.51.100.44 is listed in 42 malicious botnet blocklists; 0 logins succeeded.",
    analystAction:
      "Document indicators of compromise (IOCs), escalate to Tier-2 incident response if host was breached.",
  },
  {
    step: 6,
    ...
    exampleData:
      "ACTION_EXECUTED: iptables -A INPUT -s 198.51.100.44 -j DROP; ticket closed as CONTAINED.",
    analystAction:
      "Verify containment effectiveness and feed new threat intelligence indicators into detection rules.",
  },
];
      ...
      <div className="flex items-center justify-between border-b border-border/80 pb-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Trace how a raw security event transforms from initial log ingestion into an
            analyst-verified incident response.
          </p>
        </div>
        ...
        <div className="rounded-xl border border-primary/30 bg-muted/20 p-5 space-y-4 animate-in fade-in">
          <div className="flex items-center justify-between">
            <h5 className="font-display font-bold text-base text-foreground flex items-center gap-2">
              ...
              <span>
                Step 0{current.step}: {current.name}
              </span>
            </h5>
          </div>
