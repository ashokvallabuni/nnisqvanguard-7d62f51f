...
export const linuxSshBruteforceLab: LabDefinition = {
  ...
  tasks: [
    ...
    },
    {
      ...
      validate: (ctx, cmd) => {
        return cmd.includes("cat") && cmd.includes("flag.txt") && ctx.stdout.join("").includes("FLAG{");
      }
    }
  ],
  setupFilesystem: () => {
    ...
    vfs.writeFile("/home/analyst/README.txt", "Investigate /var/log/auth.log to find the attacker.\n");
