...
const WEIGHTS: Record<
  keyof z.infer<typeof Answers>,
  { w: number; safeIsTrue: boolean; label: string }
> = {
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          a = Answers.parse(await request.json());
        } catch (e) {
        ...
        const strengths: string[] = [],
          weaknesses: string[] = [];
        for (const [k, cfg] of Object.entries(WEIGHTS) as Array<
          [keyof typeof WEIGHTS, (typeof WEIGHTS)[keyof typeof WEIGHTS]]
        >) {
          const v = a[k];
