    ...
    <main className="min-h-screen bg-[#112240] text-slate-100">
      ...
      <section className="px-4 md:px-8 py-16">
        <div className="max-w-6xl mx-auto grid lg:grid-cols-[1.2fr_0.8fr] gap-12 items-start">
          ...
          <div className="glass backdrop-blur-xl bg-slate-900/75 border border-slate-800 rounded-2xl p-6 md:p-10 shadow-2xl">
            <h2 className="text-2xl font-display font-bold text-white mb-8 border-b border-slate-800 pb-4">Request a Briefing</h2>
            ...
            <form className="space-y-6" onSubmit={(e) => { e.preventDefault(); alert('Consultation request submitted securely.'); }}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                  ...
                  <input type="text" className="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-cyan-500 transition placeholder:text-slate-600" placeholder="Jane Doe" required />
                </div>
                <div>
                  <label className="font-mono text-[0.65rem] text-cyan-400 mb-2 block uppercase tracking-wider">Official Email</label>
                  <input type="email" className="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-cyan-500 transition placeholder:text-slate-600" placeholder="jane@organization.com" required />
                </div>
              </div>

              <div>
                <label className="font-mono text-[0.65rem] text-cyan-400 mb-2 block uppercase tracking-wider">Organization Name</label>
                <input type="text" className="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-cyan-500 transition placeholder:text-slate-600" placeholder="Acme Corp" required />
              </div>

              <div>
                <label className="font-mono text-[0.65rem] text-cyan-400 mb-2 block uppercase tracking-wider">Area of Interest</label>
                <select 
                  className="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-cyan-500 transition"
              ...
              <div>
                ...
                <textarea rows={4} className="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-cyan-500 transition placeholder:text-slate-600" placeholder="Briefly describe your objectives or current challenges..." required></textarea>
              </div>
          ...
          <div className="space-y-6">
            <div className="glass backdrop-blur-xl bg-slate-900/75 border border-slate-800 rounded-2xl p-8 shadow-2xl">
