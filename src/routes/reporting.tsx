          ...
          <div className="mb-6">
            ...
            {step < 4 && (
              <div className="flex items-center gap-2 font-mono text-[0.65rem] text-slate-500">
                ...
                <div className="w-4 h-px bg-slate-700" />
                <div className={`px-2 py-1 rounded ${step === 3 ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : ''}`}>3. CONTACT</div>
              </div>
            )}
          </div>

          <div className="glass backdrop-blur-xl bg-slate-900/75 border border-slate-800 shadow-2xl rounded-2xl overflow-hidden transition-all duration-300">
            
            {step === 1 && (
              <div className="p-6 md:p-8 animate-in fade-in slide-in-from-right-4">
                <h2 className="text-xl font-bold mb-6 text-white font-display">What are you reporting today?</h2>
                    ...
                    <button
                      ...
                      className={`text-left p-5 rounded-xl border transition-all ${
                        form.incident_type === type.id 
                          ? 'border-cyan-500 bg-cyan-500/10 shadow-[0_0_15px_rgba(0,240,255,0.1)]' 
                          : 'border-slate-800 bg-slate-950/50 hover:border-slate-600'
                      }`}
                    >
                      <div className={`font-semibold ${form.incident_type === type.id ? 'text-cyan-400' : 'text-slate-200'}`}>
                        {type.title}
                      </div>
            ...
            {step === 2 && (
              <div className="p-6 md:p-8 animate-in fade-in slide-in-from-right-4">
                <h2 className="text-xl font-bold mb-2 text-white font-display">Incident Details</h2>
                <p className="text-slate-400 text-sm mb-6">Please provide as much context as possible.</p>
                ...
                <div className="space-y-5">
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-4 py-3 text-sm text-slate-200 focus:outline-none focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/30 transition placeholder:text-slate-600"
                    />
                  </div>
                  
                  <div>
                    <label className="font-mono text-[0.65rem] text-cyan-400 mb-2 flex items-center gap-1.5 uppercase tracking-wider">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950/50 border border-slate-800 rounded-lg px-3 py-2.5 text-sm text-slate-300 file:mr-3 file:bg-slate-800 file:text-slate-200 file:border-0 file:px-3 file:py-1.5 file:rounded file:font-mono file:text-[0.7rem] file:font-semibold hover:file:bg-slate-700 transition"
                    />
                    <p className="mt-2 text-[0.65rem] text-slate-500 font-mono">
            ...
            {step === 3 && (
              <div className="p-6 md:p-8 animate-in fade-in slide-in-from-right-4">
                <h2 className="text-xl font-bold mb-2 text-white font-display">Contact Information</h2>
                <p className="text-slate-400 text-sm mb-6">Leave blank to submit anonymously, or provide details for follow-up.</p>
                
                <div className="space-y-4">
                  <div>
                    <label className="font-mono text-[0.65rem] text-slate-400 mb-2 block uppercase tracking-wider">Name (Optional)</label>
                    <input
                      type="text"
                      value={form.name}
                  ...
                  <div>
                    <label className="font-mono text-[0.65rem] text-slate-400 mb-2 block uppercase tracking-wider">Email (Optional)</label>
                    <input
                      type="email"
                      value={form.email}
                  ...
                  <div>
                    <label className="font-mono text-[0.65rem] text-slate-400 mb-2 block uppercase tracking-wider">Phone (Optional)</label>
                    <input
                      type="text"
                      value={form.phone}
            ...
            {step === 4 && (
              <div className="p-6 md:p-8 animate-in zoom-in-95 duration-500 text-center">
                ...
                <h2 className="text-2xl font-bold mb-2 text-white font-display">Report Submitted</h2>
                <p className="text-slate-400 text-sm max-w-md mx-auto mb-6">
                  Your incident has been securely transmitted to our triage system.
                </p>
                
                <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 max-w-sm mx-auto mb-8">
                  <div className="text-[0.65rem] text-slate-500 font-mono uppercase tracking-widest mb-1">Generated Case ID</div>
                  <div className="text-xl font-mono font-bold text-cyan-400 tracking-wider select-all">{caseId}</div>
                  <div className="text-[0.65rem] text-slate-500 mt-2">Save this ID for your records and follow-ups.</div>
                </div>

                {/* First-Aid Resources */}
                <div className="text-left border-t border-slate-800/60 pt-6">
                  <h3 className="font-mono text-xs text-amber-500 font-bold tracking-wider mb-4 flex items-center gap-2">
                    <Activity className="w-4 h-4" /> IMMEDIATE FIRST-AID STEPS
                  </h3>
                  <div className="grid sm:grid-cols-2 gap-4">
                    <div className="p-4 rounded-xl border border-slate-800 bg-slate-950/50 flex gap-3">
                      <ShieldCheck className="w-5 h-5 text-cyan-400 shrink-0" />
                      <div>
                        <div className="font-semibold text-slate-200 text-sm">Lock Accounts</div>
                        <div className="text-xs text-slate-400 mt-1">Immediately change passwords and enable 2FA on compromised accounts.</div>
                      </div>
                    </div>
                    <div className="p-4 rounded-xl border border-slate-800 bg-slate-950/50 flex gap-3">
                      <FileText className="w-5 h-5 text-cyan-400 shrink-0" />
                      <div>
                        <div className="font-semibold text-slate-200 text-sm">Monitor Credit</div>
                        <div className="text-xs text-slate-400 mt-1">Contact your bank and place a fraud alert on your credit profile.</div>
                      </div>
                    </div>
                  </div>
                </div>
