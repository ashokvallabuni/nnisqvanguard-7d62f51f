...
async function testVerificationEngine() {
  ...
  for (const tc of targetCourses) {
    ...
    console.log(
      `  Correct Option (Server Authoritative): Index ${quiz.correct_option} -> "${quiz.options[quiz.correct_option]}"`,
    );
    ...
    console.log(
      `  Server-evaluated is_correct: ${isCorrectRight}, score: ${score}% (Expected: true, 100%)`,
    );
    ...
    const { error: progErr } = await supabaseAdmin.from("module_progress").upsert(
      {
        user_id: testUserId,
        module_id: module.id,
        completed: true,
    ...
    if (progErr) {
      console.log(
        `  Note on module_progress upsert (FK constraint on auth.users): ${progErr.message}`,
      );
    } else {
      ...
      const { data: readBack } = await supabaseAdmin
        ...
        .single();
      console.log(
        `  Verified DB Read-back: completed=${readBack?.completed}, score=${readBack?.score}%`,
      );
    }
  }
