...
async function testLockedCourseProtection() {
  ...
  console.log(`  Access check for non-admin user: isLocked = ${isLockedForUser} (Expected: true -> LOCKED UI)`);
  ...
  console.log(`  Access check after publish: isAccessible = ${isAccessibleAfterPublish} (Expected: true -> UNLOCKED UI)`);
